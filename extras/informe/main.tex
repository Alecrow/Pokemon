...
\subsection{Función objetivo y Estrategia de Solución}

El objetivo es minimizar el \textbf{costo total} del entrenamiento. Dado que las unidades de "encuentros" y "distancia en tiles" no son directamente comparables, se utiliza una función de puntuación ponderada para evaluar la idoneidad de cada zona candidata $z$ desde la posición actual $p$:

$$ \text{Score}_z = \lambda \cdot d_{p,z} + (1 - \lambda) \cdot \text{Encounters}_z \cdot K $$

Donde:
\begin{itemize}
    \item $d_{p,z}$ es la distancia mínima desde la posición actual $p$ a la zona $z$ (calculada mediante Dijkstra).
    \item $\text{Encounters}_z$ es el número estimado de combates necesarios en la zona $z$ para satisfacer la necesidad de EVs.
    \item $\lambda \in [0, 1]$ es el factor de preferencia del usuario.
    \item $K$ es un factor de escala (empíricamente $K=10$) para normalizar el peso de los encuentros frente a la distancia, dado que un combate consume significativamente más tiempo que dar un paso.
\end{itemize}

El algoritmo selecciona iterativamente la zona que minimiza este puntaje.
...
\subsection{Algoritmo Implementado: Heurística Codiciosa Iterativa}

Para resolver el problema de optimización multi-objetivo (varias estadísticas a la vez) de manera eficiente, se implementó una estrategia \textbf{Greedy (Codiciosa) Iterativa} apoyada en el algoritmo de Dijkstra.

El procedimiento es el siguiente:

\begin{enumerate}
    \item \textbf{Inicialización:} Se parte de la zona actual del jugador y el estado actual de EVs.
    \item \textbf{Cálculo de Necesidades:} En cada iteración, se calcula el déficit de EVs para cada estadística objetivo. Si no hay déficit, el algoritmo termina.
    \item \textbf{Cálculo de Distancias:} Se ejecuta el algoritmo de \textbf{Dijkstra} desde la zona actual para obtener la distancia mínima a todas las demás zonas accesibles del mapa.
    \item \textbf{Evaluación de Candidatos:} Se evalúa cada zona accesible utilizando la función de puntuación definida en 5.4. Se considera la zona solo si provee EVs para alguna de las estadísticas deficitarias.
    \item \textbf{Selección y Acción:} Se selecciona la zona con el menor puntaje (mejor balance costo-beneficio). El algoritmo simula el desplazamiento a esa zona y la realización de combates hasta satisfacer la necesidad de la estadística principal de esa zona o alcanzar un límite de seguridad.
    \item \textbf{Actualización:} Se actualiza la posición actual y los EVs acumulados. Se repite el proceso desde el paso 2.
\end{enumerate}

Este enfoque permite encontrar soluciones sub-óptimas de muy buena calidad en tiempo casi real, adaptándose dinámicamente a múltiples objetivos sin la complejidad exponencial de buscar la ruta global perfecta en un espacio de estados masivo.
...
Para resolver este problema, se han considerado y utilizado:

\begin{itemize}
    \item \textbf{Dijkstra:} Utilizado en cada paso de la heurística para calcular la matriz de distancias desde el nodo actual a todos los demás nodos del grafo. Es fundamental para evaluar el componente de costo de desplazamiento.
    \item \textbf{Heurística Greedy:} Estrategia principal para la toma de decisiones paso a paso, seleccionando localmente la mejor opción para converger a la solución global.
\end{itemize}
