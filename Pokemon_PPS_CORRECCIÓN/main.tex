...
\begin{enumerate}
    ...
    \item \textbf{Restricciones de acceso definidas por el usuario:} Para efectos de este trabajo, se asume que el mapa completo de Kanto está desbloqueado. El usuario puede definir manualmente qué zonas desea considerar en su planificación, excluyendo aquellas que no ha desbloqueado en su partida (por falta de HMs, medallas o progreso en la historia). Esta simplificación permite que el sistema sea agnóstico al estado de progreso del jugador, delegando la responsabilidad de filtrar zonas accesibles a la entrada del usuario.
    
    Formalmente, el usuario define un subconjunto $V_{\text{user}} \subseteq V$ de zonas disponibles, y el algoritmo opera únicamente sobre el subgrafo inducido $G_{\text{user}} = (V_{\text{user}}, E_{\text{user}})$ donde:
